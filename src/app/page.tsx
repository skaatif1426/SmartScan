...
export default function ScannerPage() {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Create the hidden scanner instance to read the file
    try {
  ...
  return (
    <>
      ...
      {/* Hidden Infrastructure - These MUST stay in the DOM regardless of state to avoid ReferenceErrors during async file scanning */}
      <input 
        type="file" 
        ref={fileInputRef} 
      ...
      />
      {/* 
        This div must be in the DOM and have dimensions for html5-qrcode's scanFile to work.
        We use visibility/opacity instead of 'display: none' (Tailwind hidden) to ensure it has clientWidth/Height.
      */}
      <div 
        id="reader-hidden" 
        className="fixed top-0 left-0 w-[100px] h-[100px] pointer-events-none opacity-0 -z-50" 
      />
    </>
  );
}
